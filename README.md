...
这是面向 **Codex 和 Claude Code** 的项目级插件目录：按需收录第三方技能与 Hook，并维护两端可读取的配置。当前覆盖软件工程工作流、精简实现与代码审计、界面设计与动效、React/Next.js 性能、TypeScript 高级类型、Python 与应用性能优化、后端 API 设计、Postgres 实践、可观测性及系统迁移；尚未配置 MCP 服务，也未整仓镜像任何上游项目。
...
| [Addy Osmani · Agent Skills](https://github.com/addyosmani/agent-skills/blob/main/README.md) | 上游按需求、计划、构建、验证、评审和交付组织工程技能；本项目收录性能优化、API 与接口设计、可观测性、系统退役与迁移、代码简化，以及按正确性、可读性、架构、安全和性能审查的 `code-review-and-quality`。 | 已收录 6 项技能及各技能所需的检查清单；未引入上游命令、Agent 或 Hook |
